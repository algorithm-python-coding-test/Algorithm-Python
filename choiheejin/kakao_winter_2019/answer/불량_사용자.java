...
public class 불량_사용자 {
    public int solution(String[] user_id, String[] banned_id) {
        ...
        dfs(checked, banned_user, 0, cases);
        return cases.size();
    }

    private void dfs(boolean[] checked, List<List<Integer>> banned_user, int i, Set<Set<Integer>> cases) {
        for (int j = 0; j < banned_user.get(i).size(); j++) {
            if (!checked[banned_user.get(i).get(j)]) {
                ...
                if (i == banned_user.size() - 1) {
                    ...
                    dfs(checked, banned_user, i + 1, cases);
                }
